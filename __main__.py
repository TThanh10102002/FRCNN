...
from .static import TrainingStatistics
from .static import PrecisionRecallCurveCalculator
...
def _sample_rpn_minibatch(rpn_map, obj_indices, background_indices, rpn_minibatch_size):
    """
    ...
    assert len(obj_indices) == 1, "Batch size must be 1"
    assert len(background_indices) == 1, "Batch size must be 1"
    pos_anchors = obj_indices[0]
    neg_anchors = background_indices[0]
    assert len(pos_anchors) + len(neg_anchors) >= rpn_minibatch_size, "Image has insufficient anchors for RPN minibatch size of %d" % rpn_minibatch_size
    assert len(pos_anchors) > 0, "Image does not have any positive anchors"
    ...
    pos_anchors = pos_anchors[pos_anchor_indexes]
    neg_anchors = neg_anchors[neg_anchor_indexes]
    trainable_anchors = np.concatenate([pos_anchors, neg_anchors])
    batch_indexes = np.zeros(len(trainable_anchors), dtype = int)
    trainable_indexes = (batch_indexes, trainable_anchors[:, 0], trainable_anchors[:, 1], trainable_anchors[:, 2], 0)

    #Create a copy of the RPN map with samples set as trainable
    rpn_minibatch_map = rpn_map.copy()
    rpn_minibatch_map[:, :, :, :, 0] = 0
    rpn_minibatch_map[:, :, :, :, 1] = 1
...
def _convert_training_sample_to_model_input(sample, mode):
    ...
    if mode == "train":
        x = [img_data, anchor_map, anchor_valid_map, gt_rpn_minibatch_map, gt_box_class_indexes, gt_box_corners]
    else:
        x = [img_data, anchor_map, anchor_valid_map]

    #Return all plus some unpacked elements for convenience
    return x, img_data, gt_rpn_minibatch_map

def evaluate(model, eval_data = None, num_samples = None, plot = False, print_avg_precisions = False):
    ...
    for sample in tqdm(iterable = iter(eval_data), total = num_samples):
        x, img_data, _ = _convert_training_sample_to_model_input(sample = sample, mode = "infer")
        scored_boxes_by_class_index = model.predict_on_batch(x = x, score_threshold = 0.05)         #lower threshold score for evaluation
        precision_recall_curve.add_img_results(
        ...
        if i >= num_samples:
            break
    if print_avg_precisions:
        precision_recall_curve.print_average_precisions(class_index_to_name = voc.Dataset.class_index_to_name)
    mean_avg_precision = 100.0 * precision_recall_curve.compute_mean_average_precision()
    print("Mean Average Precision = %1.2f%%" % mean_avg_precision)
    if plot:
...
def train(model):
    ...
    if options.save_best_to:
        best_weights_tracker = utils.BestWeightTracker(filepath = options.save_best_to)
    for epoch in range(1, 1 + options.epochs):
        ...
        mean_avg_precision = evaluate(
            model = model,
            eval_data = eval_data,
            num_samples = options.periodic_eval_samples,
            plot = False,
            print_avg_precisions = False
        )
        if options.checkpoint_dir:
            checkpoint_file = os.path.join(options.checkpoint_dir, "checkpoint-epoch-%d-mAP-%1.1f.h5" % (epoch, mean_avg_precision))
    ...
    evaluate(
        ...
        num_samples = eval_data.num_samples,
        plot = options.plot,
        print_avg_precisions = True
    )

def _predict(model, img_data, img, show_img, output_path):
    anchor_map, anchor_valid_map = anchors.generate_anchor_maps(img_shape = img_data.shape, f_pixels = 16)
    ...
    img_data = np.expand_dims(img_data, axis = 0)                                               #convert to batch size of 1: (1,height,width,3)
    img_shape_map = np.array([[img_data.shape[1], img_data.shape[2], img_data.shape[3]]])       #(1,3), with (height,width,channels)
    x = [img_data, anchor_map, anchor_valid_map]
    scored_boxes_by_class_index = model.predict_on_batch(x = x, score_threshold = 0.7)
    visualization.show_detections(
        output_path = output_path,
        show_img = show_img,
        img = img,
        scored_boxes_by_class_index = scored_boxes_by_class_index,
...
def predict_one(model, url, show_img, output_path):
    from .datasets.images import load_img
    img_data, img, _, _ = load_img(url = url, min_dim_pixels = 600)
    _predict(model = model, img_data = img_data, img = img, show_img = show_img, output_path = output_path)
...
def predict_all(model, split):
    ...
    for sample in iter(dataset):
        ...
        _predict(model = model, img_data = sample.img_data, img = sample.img, show_img = False, output_path = output_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("FRCNN")
    ...
    group.add_argument("--predict", metavar = "url", action = "store", type = str, help = "Run inference on image and display boxes")
