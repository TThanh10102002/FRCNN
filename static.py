...
class TrainingStatistics:
    ...
    def __init__(self):
        ...
        self._rpn_reg_losses = []
        self._detector_class_losses = []
        self._detector_reg_losses = []
    ...
    def on_training_step(self, losses):
        ...
        self.detector_reg_loss = np.mean(self._detector_reg_losses)
...
class PrecisionRecallCurveCalculator:
    ...
    def _compute_correctness_of_predictions(self, score_boxes_by_class_index, gt_boxes):
        ...
        for class_index, scored_boxes in score_boxes_by_class_index.items():
    ...
    def add_img_results(self, scored_boxes_by_class_index, gt_boxes):
        """
        ...
        unsorted_predictions_by_class_index, object_count_by_class_index = self._compute_correctness_of_predictions(
            score_boxes_by_class_index = scored_boxes_by_class_index,
            gt_boxes = gt_boxes
        )
    ...
    def _compute_average_precision(self, class_index):
        ...
        num_ground_truth_pos = self._object_count_by_class_index[class_index]

        #Compute raw recall and precision arrays
        recall_array = []
        precision_array = []
        true_pos = 0        #running tally
        false_pos = 0       #""
        for i in range(len(sorted_predictions)):
            true_pos += 1 if sorted_predictions[i][1] == True else 0
            false_pos += 0 if sorted_predictions[i][1] == True else 1
            recall = true_pos / num_ground_truth_pos
